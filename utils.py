...
def get_random_patches(img, patch_size, num_patches):
    (h, w) = patch_size
    (H, W) = img.shape[:2]
    ...
    mu = (H - h - 1) // 2
    sigma = mu // 2
    ys = stats.truncnorm((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma).rvs(num_patches)

    lower, upper = 0, W - w - 1
    mu = (W - w - 1) // 2
    sigma = mu // 2
    xs = stats.truncnorm((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma).rvs(num_patches)
        
    patches = []
    for y1, x1 in zip(ys, xs):
        patches.append(img[y1:y1+h, x1:x1+w, :])
    return patches_coords, patches

def draw_patches(img, patches_coords):
    img = img.copy()
